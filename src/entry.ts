...
// check breaks under the npm bin symlink (…/bin/ghrelay → …/dist/cli.js): argv1
...
export function shouldRunAsEntry(
  ...
  if (importMetaMain === undefined && shouldForceEntry(argv1, binNames)) {
    return {
      run: true,
      warning: `${binNames[0]}: entry detection failed — treating as main; report at github.com/gabros20/github-relay/issues`,
    };
  }
