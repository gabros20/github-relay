...
export const COMMANDS: CommandDef[] = [
  ...
  },
  {
    ...
    summary: 'Run many search queries from a file, serialized with a delay, merged into one corpus.',
    usage: 'ghrelay batch --file queries.txt --out corpus.json [--delay 2000] [--dry-run]',
  },
  {
    name: 'hydrate',
    cost: '~1 GraphQL point per 50 ids',
    summary: 'Ingest candidate owner/repo ids the agent found elsewhere (web search, awesome lists).',
    usage: 'ghrelay hydrate <owner/repo...> [--out corpus.json] [-]',
  },
  {
    name: 'code',
    cost: 'free — grep.app lane',
    summary: 'Code-token/regex evidence search across ~1M top repos. Not for natural-language intent.',
    usage: 'ghrelay code <pattern> [--lang X --repo o/r --path P] [--limit 20]',
  },
  {
    name: 'enrich',
    cost: '~2-4 GraphQL points per 50 repos + zero-quota third parties',
    summary: 'Deepen a corpus with the ~15-signal fragment plus the ecosyste.ms/deps.dev fallback chain.',
    usage: 'ghrelay enrich --in corpus.json [ids...] [--top 50] [--skip-deps] [--stale-ok]',
  },
  {
    name: 'rank',
    cost: 'free — offline, zero network',
    summary: 'Score a corpus offline on 7 signal groups; compact rows with coverage + explainability.',
    usage:
  ...
  },
  {
    ...
    summary: 'Forensics on finalists — issue latency, star-velocity burstiness, bus factor. Re-scores C/D/F.',
  ...
  },
  {
    ...
    summary: 'Read specific files by path, content-addressed and cached; missing path is a clean ok:true.',
    usage: 'ghrelay read <owner/repo> <paths...> [--ref SHA] [--max-chars 6000]',
  },
  {
    name: 'digest',
    cost: 'expensive — 1 tarball request (or 0-quota blobless clone)',
    summary: 'Full gitingest-style repo digest, filtered and token-capped. Only for confirmed finalists.',
    usage:
  ...
  },
  {
    ...
    summary: 'Report remaining headroom across every pool (GraphQL, REST, ecosyste.ms, OSS Insight, grep.app).',
  ...
  },
  {
    name: 'cache',
    cost: 'free, local',
    summary: 'Inspect or clear the local ~/.ghrelay cache (etags, blobs, trees, tarballs, corpora).',
