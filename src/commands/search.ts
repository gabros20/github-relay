...
/** Only the surface search actually calls — narrower than `Pick<Sources,'ghGraphql'|'ghRest'>`, which would still demand each adapter's full declared shape. */
...
export async function runSearch(
  ...
  if (source === 'trending') {
    throw new EngineError(
      'INVALID_INPUT',
      'search --source trending lands in task 12 (design §10 milestone B); use --source gh or --source rest for now',
    );
  }
  ...
  if (opts.out) {
    const existing = loadCorpusOrEmpty(opts.out, q, now);
    const fresh: Corpus = {
    ...
    result.repos = opts.fields ? rows.map((r) => projectFields(r, opts.fields as string)) : rows;
  }
